...
public class OrderAddController extends Controller {
	...
	public void save() {	
	   ...
	   orderMain.save();
	   //细表数量
	   String[]  table_sl = getParaValues("table_sl");//数量数量
	...
	//   String[] table_je = getParaValues("table_je");
	   ...
	   for(int i=0;i<table_spbh.length;i++){
		   ...
		   String tmp_sl1= "0";
	//	   String tmp_sl2= "0";
		   if(null !=table_sl  && !"".equals(table_sl[i])){
			   tmp_sl1 = table_sl[i];
		   }
//		   if(null !=table_sl2  && !"".equals(table_sl2[i])){
//			   tmp_sl2 = table_sl2[i];
//		   }
		   makeOrderDetail(tmp,djbh,String.valueOf((i+1)),tmp_sl1).save();  
	   }
	   redirect("/order");
	}
	...
	public OrderDetail makeOrderDetail(Pharmaceutical pharmaceutical,String djbh,String dj_sort,String shl){
		
		double baozhshl = 0.00;
		double lingsshl = 0.00;
		double je = 0.00;
		double hsje = 0.00;
		double lshje = 0.00;
		...
		if(!"".equals(shl)&& !"0".equals(shl)){
			Integer jlgg = Integer.parseInt(String.valueOf(pharmaceutical.get("jlgg")));
			baozhshl  = Double.parseDouble(shl)/jlgg;
			lingsshl  = Double.parseDouble(shl)%jlgg;
		}
